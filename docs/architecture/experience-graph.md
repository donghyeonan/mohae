# Experience Graph — minimum future DB split

Do not model the flywheel as one mutable favorites table.
...
4. `experience_events`: append-only `exposed`, `passed`, `saved`, `map_opened`, `extended`, `attended`, and `reviewed` events.
...
This keeps the UI small while preserving the sequence needed to learn `recommendation → save → attendance` without treating a missing review as dissatisfaction.
